...
def define_arguments():
    ...
    flags.DEFINE_string(
        "input_pdb", None, required=True, help="pdb id to pdb file path, csv"
    )
    flags.DEFINE_string(
        "output_records", None, required=True, help="metal binding chains file, csv"
    )


def run(dict_pdb_file: dict, df: pd.DataFrame):
    records = []
    for pdb_chain, df_chain in df.groupby(
        ["pdb", "metal_chain"]
        ...
        homo_chains = set(
            [k for k, v in ndb_seqs_can.items() if v == ndb_seqs_can[chain]]
                ...
                records.append(
                    {
                        ...
                        "resi_pdb_id": (
                            " ",
                            row["resi_pdb_seq_num"],
                            row["resi_pdb_ins_code"],
                        ),
                    }
                )
    return records
...
def main(argv):
    ...
    df_input = pd.read_csv(
        input_records, sep="\t", na_values=[], keep_default_na=False
    )  # do not parse 'NA' chain as pd.nan
    df_input["metal_chain"] = df_input["metal_chain"].astype(str)

    num_task = mp.cpu_count()
    df_chains = split_data_frame_by_id(df_input, id=["pdb", "metal_chain"], n=num_task)
    jobs = mp.Pool(num_task)
    result = [jobs.apply_async(run, args=(dict_pdb_file, pg)) for pg in df_chains]
