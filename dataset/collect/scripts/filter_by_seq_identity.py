...
def define_arguments():
    flags.DEFINE_string(
        ...
        None,
        required=True,
        help="fasta file for cluster, pdb_chain as id",
    )
    flags.DEFINE_string(
        "input_pdb",
        None,
    ...
    flags.DEFINE_string(
        "input_records", None, required=True, help="metal binding sites file, csv"
    )
    flags.DEFINE_string(
        "output_records",
        None,
        required=True,
        help="non-redundant metal binding sites file, csv",
    )
    flags.DEFINE_string(
        "output_fasta",
        None,
        required=True,
        help="fasta file for nr metal binding chains",
    )
    flags.DEFINE_string(
        "output_pdb",
        None,
...
def get_clusters(fasta_file: str, seq_id: float = 0.3):
    ...
    result = os.system(
        f"mmseqs easy-cluster {fasta_file} result tmp --min-seq-id {seq_id} -v 1"
    )
    clusters = []
    if result == 0:
        df_cluster = pd.read_csv(
            "./result_cluster.tsv", sep="\s+", header=None, names=["repr", "member"]
        )
...
def select_chain_from_cluster(
    ...
    if num_sites_x > num_sites_y:
        return 1
    elif num_sites_x < num_sites_y:
        return -1
    else:
        ...
        if res_x < res_y:
            ...
            if date_x > date_y:
                ...
                return 1 if chain_x > chain_y else -1
...
def filter_by_seq_identity(
    ...
    for cluster in clusters:
        ...
        target_pdb_chain = max(
            cluster,
            key=cmp_to_key(
                partial(
                    select_chain_from_cluster,
                    dict_num_sites=dict_num_sites,
    ...
    df_selected = df_sites[
        df_sites.apply(
            lambda row: (row["pdb"], row["metal_chain"]) in selected_nr_chains, axis=1
        )
    ]
    return df_selected
...
def main(argv):
    ...
    dict_pdb_files = dict(
        zip(
            df_pdb["pdb"],
            zip(df_pdb["pdb_file"], df_pdb["resolution"], df_pdb["deposition_date"]),
        )
    )
    df_output = filter_by_seq_identity(
        clusters, dict_pdb_files, pd.read_csv(input_records, sep="\t")
    )
