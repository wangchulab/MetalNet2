...
def get_ndb_seq_can_num_rcsb(mmcif_dict: MMCIF2Dict) -> dict:
    ...
    for chain in ndb_seqs.keys():
        try:  # only consider sequence without deoxynucleotides
            ndb_seqs_three_letter[chain] = get_ndb_seq_three_letter(ndb_seqs[chain])
    ...
    for chain, df_chain in df.groupby(["pdb_strand_id"]):
        ...
        for _, row in df_chain.iterrows():
            pdb_seq_num, pdb_ins_code, pdb_mon_id = (
                row["pdb_seq_num"],
                row["pdb_ins_code"],
                row["pdb_mon_id"],
            )
            ...
            mon_id = str.upper(
                ndb_seq_three_letter[
                    ndb_seq_three_letter_posi : (ndb_seq_three_letter_posi + 3)
                ]
            )
            if (
                mon_id == pdb_mon_id
            ):  # usually it's equal; if hetero residue exsits, only when pdb resi match ndb_seq resi, e.g., 6dtk in ndb_num 84 and 253
                ndb_seq_posi = ndb_seq_num - 1
                ndb_seq_can_posi = (
                    align_seq_to_seq_can[ndb_seq_posi]
                    if ndb_seq_posi in align_seq_to_seq_can.keys()
                    else -1
                )
