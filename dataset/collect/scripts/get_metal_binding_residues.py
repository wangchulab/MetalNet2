...
def define_arguments():
    ...
    flags.DEFINE_string(
        "output", None, required=True, help="metal binding sites file, csv"
    )
    flags.DEFINE_float(
        "interacted_distance",
        3.0,
    ...
    flags.DEFINE_bool(
        ...
        False,
        help="for single metal ion, only consider Zn, Ca, Mg, Mn, Fe, Cu, Ni, Co",
    )
    flags.DEFINE_bool(
        "include_metal_compound", False, help="take SF4, FES and F3S into consideration"
    )
    flags.DEFINE_bool(
        "include_main_chain", False, help="allow main chain O and N atom as ligand"
    )
    flags.DEFINE_bool("include_water", False, help="allow water as ligand")
    flags.DEFINE_bool(
        "use_sloppy_mode",
        False,
        help="sloppy mode, if True, do not consider has_coordinate_residue_on_metal_chain, \
...
def select_metal_binding_sites(
        ...
        if hetflag == " ":
            if str.capitalize(r.get_resname()) in common_amino_acids:
                ...
                if include_main_chain:
                    atoms.extend(
                        [a for a in r.get_atoms() if a.element in hetero_atoms]
                    )
                else:
                    atoms.extend(
                        [
                            ...
                            if a.element in hetero_atoms
                            and a.get_name() not in main_chain_atom_names
                        ]
                    )
            else:
                ...
                if len(elements) == 1:
                    ...
                    if hetflag[2:] in common_metal_compound_names:
                        if include_metal_compound:
                            metal_residues.append(
                                r
                            )  # second allowed metal reisude type
    ...
    for mr in metal_residues:
        ...
        for ma in mr.get_atoms():
            ...
            if str.capitalize(ma.element) in metal_elements:
                atoms = set(
                    ns.search(ma.get_vector().get_array(), interacted_distance, "A")
                )
                for a in atoms:
                    a: Atom
                    ...
                    if (
                        hetflag == " "
                        and str.capitalize(r.get_resname()) in common_amino_acids
                    ):
                        valid_coordinate_atoms.add((a, a - ma))
            ...
            if hetflag != " ":
                if hetflag == "W" or hetflag == "H_HOH":
                    if not include_water:
                        has_unexpected_coordinate_residue = True
                else:
                    if r != mr:
                        has_unexpected_coordinate_residue = True
            else:
                if str.capitalize(r.get_resname()) not in common_amino_acids:
                    has_unexpected_coordinate_residue = True
                else:
                    # check second part
                    if r.get_full_id()[2] == mr.get_full_id()[2]:
                        has_coordinate_residue_on_metal_chain = True
                    # check thrid part
                    if r.is_disordered():
                        has_disordered_entities = True
                    valid_coordinate_residues.add(
                        r
                    )  # if use sloppy mode, disordered residues are counted

        if not use_sloppy_mode:
            if not has_coordinate_residue_on_metal_chain:
                continue
            if has_disordered_entities or has_unexpected_coordinate_residue:
                continue
    ...
    for k, v in metal_to_coordinate_atoms.items():
        ...
        for atom, distance in v:
            ...
            _, resi_ndb_seq_num, resi_ndb_seq_can_num = ndb_seq_can_num_dict[
    ...
    **kwargs,
    ...
    for pdb_id, pdb_file in dict_pdb_file.items():
        try:
            ...
            logging.error(
                f"Failed selecting metal binding sites in protein with pdb id: {pdb_id}"
            )
...
def main(argv):
    ...
    pdb_groups = [
        dict(list(pg))
        for pg in divide(num_task, dict(zip(df_pdb["pdb"], df_pdb["pdb_file"])).items())
    ]
